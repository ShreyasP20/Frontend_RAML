                ...
                <div class="report-actions">
                    ${
                      report.status === "Completed"
                        ? `<button class="download" data-id="${report.id}">Download Report</button>`
                        : ""
                    }
...
generateBtn.addEventListener("click", async () => {
  try {
    ...
    if (data) {
      alert(
        "APK uploaded! Analysis started. Reports will appear in the dashboard."
      );
    }
  } catch (err) {
    console.error(err);
